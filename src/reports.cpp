...
static config unit_moves(reports::context & rc, const unit* u)
{
	...
	std::vector<std::pair<std::string, int>> terrain_moves;
	...
	for (; terrain_it != preferences::encountered_terrains().end();
		...
		if (info.union_type().size() == 1 && info.union_type()[0] == info.number() && info.is_nonnull()) {
			terrain_moves.push_back(std::make_pair(info.name(), u->movement_cost(terrain)));
		}
	}

	// sort terrain list by movement cost, in same way as in unit_preview_pane.cpp
	struct
	{
	...
	} movement_compare_ascending;

	std::sort(terrain_moves.begin(), terrain_moves.end(), movement_compare_ascending); 

	for(unsigned int i = 0; i < terrain_moves.size(); i++)
	{
		tooltip << terrain_moves[i].first << ": ";
		
		std::string color;
		//movement  -  range: 1 .. 5, movetype::UNREACHABLE=impassable
		const bool cannot_move = terrain_moves[i].second > u->total_movement();
		if (cannot_move)		// cannot move in this terrain
			color = "red";
		else if (terrain_moves[i].second > 1)
			color = "yellow";
		else
			color = "white";
		...
		if(cannot_move && (terrain_moves[i].second > u->total_movement() + 5)) {
			tooltip << font::unicode_figure_dash;
		} else {
			tooltip << terrain_moves[i].second;
		}
		tooltip << naps << '\n';
	}
